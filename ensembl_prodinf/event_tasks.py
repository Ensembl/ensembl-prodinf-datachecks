...
logger = reporting.get_logger(pool, cfg.report_exchange, 'event_processing', None, {})
...
def process_result(self, event, process, job_id):
    ...
    """
    reporting.set_logger_context(logger, event['genome'], event)

    # allow infinite retries
    self.max_retries = None
    logger.info("Checking {} event {}".format(process, job_id))
    ...
    if (result['status'] == 'incomplete') or (result['status'] == 'running') or (result['status'] == 'submitted'):
        logger.info("Job incomplete, retrying")
        raise self.retry()

    logger.debug("Handling result for " + json.dumps(event))

    if result['status'] == 'failure':
        logger.fatal("Event failed: "+json.dumps(result))
    else:
        logger.info("Event succeeded: "+json.dumps(result))
        # TODO
        # 1. update metadata
