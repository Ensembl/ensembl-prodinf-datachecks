...
function downloadFormat(value, row) {
    ...
    return `<a title="Dowload DC.. " style="cursor:${cursor_download}; ${download_event}"  href="${script_name}/download_datacheck_outputs/${row.id}">${row.id}</a>`;
}
...
function detailPage(value, row) {
    ...
    return `<a title="More details " style="cursor: pointer"  >${row.id}</a>
            <a title="Download Datacheck output " style="cursor: pointer" href="${script_name}/download_datacheck_outputs/${row.id}">${download}</a>`;
}
...
function parse_details(db_name, result) {
    ...
    $.each(result, function (species, value) {
        ...
        $.each(value, function (datachecks, value1) {
            ...
            if (value1["ok"] === 0) {
                datachecks_failed++;
                card_header = "border-danger ";
                datacheck_bordear = "border-danger ";
            }
            ...
            $.each(value1["tests"], function (each_test, value2) {
                ...
                if (each_test.startsWith("not")) {
                    table_success = 'table-danger';
                }
        ...
        let failed_badge = '';
        if (datachecks_failed > 0) {
            failed_badge = '<span class="badge badge-danger">Failed:' + datachecks_failed + '</span>';
        }
